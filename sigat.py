...
def run( added_info, dataset, k=2):
    ...
    adj_additions3 = [defaultdict(set) for _ in range(16)]
    ...
    if added_info:
        ...
        betweenness_centrality, closeness_centrality, alpha_centrality, clustering_coefficient = fea_model.compute_centralities_and_clustering(graphs)
        for node, neighbors in graph.items():
            for neighbor in neighbors:
...
def main():
    ...
    run(added_info= args.added_info, dataset=args.dataset, k=K)
